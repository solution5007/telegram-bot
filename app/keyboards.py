...
def admin_payments_pagination(page: int, total: int) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        builder.row(InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin_payments_{page - 1}"))
    if (page + 1) * 5 < total:
        builder.row(InlineKeyboardButton(text="➡️ Далее", callback_data=f"admin_payments_{page + 1}"))
